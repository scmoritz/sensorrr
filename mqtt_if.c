...
struct Node
{
    ...
    char topicName[MQTT_TOPIC_SIZE];
//    struct Node* next;
};
...
//    struct Node* head;
    struct Node topicList[MQTT_NUM_TOPICS];
    unsigned int topicCount;
    int moduleState;
    uint8_t clientDisconnectFlag;
} mMQTTContext = {NULL, NULL, NULL, NULL, NULL, 0, MQTT_STATE_IDLE, 0};

int deleteNodeFromArray(struct Node *array, unsigned int arrayCount, unsigned int arrayIndex)
{
    int success = 0;
    ...
    for (; arrayIndex < arrayCount - 1; arrayIndex++)
    {
...
{
    dbgOutputLoc(DBG_ENTER_MQTTHELPERTOPICMATCHING);
    ...
    dbgOutputLoc(DBG_EXIT_MQTTHELPERTOPICMATCHING);
    return (p_next)?0:1;
}
...
{
    dbgOutputLoc(DBG_ENTER_MQTTCLIENTCALLBACK);
    int status;
//    struct MQTTMsgQueue queueElement;
    ...
    switch((MQTTClient_EventCB)event)
    {
        case MQTTClient_OPERATION_CB_EVENT:
        {
            switch(((MQTTClient_OperationMetaDataCB *)metaData)->messageType)
            {
                case MQTTCLIENT_OPERATION_CONNACK:
                {
                    ...
                    sendToMQTTQueueFromISR(MQTT_EVENT_CONNACK);
                    break;
                }
                ...
                case MQTTCLIENT_OPERATION_EVT_PUBACK:
                {
                    ...
                    sendToMQTTQueueFromISR(MQTT_EVENT_PUBACK);
                    break;
                }
                ...
                case MQTTCLIENT_OPERATION_SUBACK:
                {
                    ...
                    sendToMQTTQueueFromISR(MQTT_EVENT_SUBACK);
                    break;
                }
                ...
                case MQTTCLIENT_OPERATION_UNSUBACK:
                {
                    ...
                    sendToMQTTQueueFromISR(MQTT_EVENT_UNSUBACK);
                    break;
                }
            }
            break;
        ...
        case MQTTClient_RECV_CB_EVENT:
        {
            ...
            if (receivedMetaData->topLen >= MQTT_TOPIC_SIZE)
            {
                // topic is too long, fail
                dbgFailRoutine();
            }
            ...
            if (dataLen >= MQTT_PAYLOAD_SIZE)
                {
                    // payload is too long, fail
                    dbgFailRoutine();
                }
//            memcpy(queueElement.payload, data, dataLen);
            ...
            if(mMQTTContext.topicCount > 0){
                ...
                while(ret == 0){
                    tmp = mMQTTContext.topicList[topicIndex];
                    // TODO: Check this works with my queue
                    ret = MQTTHelperTopicMatching(tmp.topicParams.topic, topic);
                    if(ret == 1){
                    ...
                    topicIndex++;
                    if(topicIndex >= mMQTTContext.topicCount){
...
//            queueElement.event =   MQTT_EVENT_RECV;
        ...
        case MQTTClient_DISCONNECT_CB_EVENT:
        {
            ...
            sendToMQTTQueueFromISR(MQTT_EVENT_SERVER_DISCONNECT);
            break;
        }
    }
...
//    status = sendToMQTTQueueFromISR(queueElement.event, queueElement.topic, queueElement.payload);
//    if(status == 0){
//        //LOG_ERROR("msg queue send error %d", status);
//    }
    dbgOutputLoc(DBG_EXIT_MQTTCLIENTCALLBACK);
}
...
{
    dbgOutputLoc(DBG_ENTER_MQTTAPPTHREAD);
    int ret;
    int queueElement;

    while(1)
    {
        dbgOutputLoc(DBG_BEFORE_MQ_RECEIVE);
//        mq_receive(mMQTTContext.msgQueue, (char*)&queueElement, sizeof(struct msgQueue), NULL);
        readMsgFromMQTTQueue(&queueElement);
        dbgOutputLoc(DBG_AFTER_MQ_RECEIVE);
...
//        // when MQTT_IF_Deinit is called we must close the message queue and terminate the MQTTAppThread
        /*else */if(queueElement == MQTT_EVENT_DESTROY)
        {
            //LOG_TRACE("MQTT APP THREAD: DESTROY\r\n");

            mMQTTContext.eventCB(queueElement);

            // TODO: Replace closing the queue?
//            ret = mq_close(mMQTTContext.msgQueue);
        ...
        else if(queueElement == MQTT_EVENT_SERVER_DISCONNECT){
        ...
        else if(queueElement == MQTT_EVENT_CONNACK){
...
//                struct Node tmp;
                ...
                while(topicIndex < mMQTTContext.topicCount){

                    curr = mMQTTContext.topicList[topicIndex];
//                    tmp = curr;
...
//                        }
                        ...
                        deleteNodeFromArray(mMQTTContext.topicList, mMQTTContext.topicCount, topicIndex);
                        mMQTTContext.topicCount--;
...
{
    dbgOutputLoc(DBG_ENTER_MQTTCONTEXTTHREAD);
    int ret;
    ...
    dbgOutputLoc(DBG_EXIT_MQTTCONTEXTTHREAD);
    pthread_exit(0);
...
{
    dbgOutputLoc(DBG_ENTER_MQTT_INIT);
    int ret;
    mq_attr attr;
...
//    }
    ...
    dbgOutputLoc(DBG_EXIT_MQTT_INIT);
    return ret;
}

int MQTT_IF_Deinit(MQTTClient_Handle mqttClientHandle)
{
    dbgOutputLoc(DBG_ENTER_MQTTDEINIT);
    int ret;
    struct MQTTMsgQueue queueElement;
    ...
    ret = sendToMQTTQueueFromISR(MQTT_EVENT_DESTROY);
...
//    ret = sendToMQTTQueue(queueElement.event, queueElement.topic, queueElement.payload);
    if(ret == 0){
...
//    struct Node tmp = mMQTTContext.topicList[0];
    // "Erases" the topic list if we did not unsubscribe before de-initing (shouldn't ever de-init anways)
    mMQTTContext.topicCount = 0;
...
//    }
    ...
    dbgOutputLoc(DBG_EXIT_MQTTDEINIT);
...
{
    dbgOutputLoc(DBG_ENTER_MQTT_IF_CONNECT);
    int ret;
    pthread_attr_t threadAttr;
    ...
    dbgOutputLoc(DBG_EXIT_MQTT_IF_CONNECT);
    if(ret < 0){
...
{
    dbgOutputLoc(DBG_ENTER_MQTT_IF_DISCONNECT);
    int ret;
...
{
    dbgOutputLoc(DBG_ENTER_MQTT_IF_SUBSCRIBE);
    int ret = 0;
...
//        return -1;
//    }
    if (mMQTTContext.topicCount > MQTT_NUM_TOPICS)
    {
        dbgFailRoutine();
    }

    // grabs the latest position in our topicList
    topicEntry = &mMQTTContext.topicList[mMQTTContext.topicCount];
    mMQTTContext.topicCount++;
    ...
    strcpy(topicEntry->topicName, topic);
    // assigns the array pointer in the topic entry to the MQTT pointer
    topicEntry->topicParams.topic = topicEntry->topicName;
...
//            free(topicEntry);
    ...
    dbgOutputLoc(DBG_EXIT_MQTT_IF_SUBSCRIBE);
    return ret;
}

int MQTT_IF_Unsubscribe(MQTTClient_Handle mqttClientHandle, char* topic)
{
    dbgOutputLoc(DBG_ENTER_MQTT_IF_UNSUBSCRIBE);
    int ret = 0;
...
int MQTT_IF_Publish(MQTTClient_Handle mqttClientParams, char* topic, char* payload, unsigned short payloadLen, int flags)
{
    ...
    dbgOutputLoc(DBG_EXIT_MQTT_IF_PUBLISH);
